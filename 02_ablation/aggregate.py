...
- Reads results/ablation/<method>/panel<N>_h<H>/seed<S>/metrics.json
- Produces summary.csv and (optional) LaTeX table snippets.

Usage:
  python exp/01_ablation/aggregate.py --results_root results/ablation --panel 40 --horizon 5
"""
...
def main():
    ...
    ap.add_argument("--results_root", default="results/ablation")
    ...
    if df.empty:
        raise SystemExit("No results found. Did you run exp/01_ablation/run.py first?")
