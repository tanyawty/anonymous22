# exp/run.py  (stability-improved version)
# Changes vs original:
#   - Passes anchor_loss into training objective (--lambda_anchor, default 0.01)
#   - Passes entropy_weight for optional graph entropy regularisation
#   - Logs gamma value per epoch so you can watch it stay near 0.8+
#   - Adds --gamma_init, --gamma_min CLI args so you can tune from command line
#   - grad clipping (norm 1.0) for training stability
...
def train_one_seed(args, seed: int):
    ...
    def _loop(loader, train: bool):
        ...
        for x_seq, y_seq, _ in loader:
            ...
            if train:
                ...
                loss.backward()
                # grad clipping — prevents explosive updates that destabilise gamma
    ...
    for ep in range(args.epochs):
        ...
        # log gamma so you can verify it stays near 0.8
...
def main():
    ...
    p.add_argument("--mode",       required=True, help="mech | learn | prior_residual")
    p.add_argument("--price_path", required=True)
    p.add_argument("--macro_path", required=True)
    p.add_argument("--edges_path", default="", help="required for mech/prior_residual")
    ...
    p.add_argument("--gamma_init",      type=float, default=1.5,
                   help="Initial gamma_raw; sigmoid(1.5)≈0.82 biases toward mechanism")
    p.add_argument("--gamma_min",       type=float, default=0.3,
                   help="Minimum gamma — mechanism always contributes at least this much")
    p.add_argument("--lambda_anchor",   type=float, default=0.01,
                   help="Weight on graph anchor loss ||A_learn - A_mech||_F (0 = off)")
    p.add_argument("--entropy_weight",  type=float, default=0.0,
                   help="Weight on graph entropy regulariser (0 = off)")
    p.add_argument("--no_graph_norm",   action="store_true",
                   help="Disable LayerNorm on graph-learner input (not recommended)")
    # ─────────────────────────────────────────────────────────────
